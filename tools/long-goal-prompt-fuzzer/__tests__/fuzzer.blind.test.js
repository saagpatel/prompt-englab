...
describe("Long-Goal Prompt Fuzzer first holdout regression", () => {
  it("fails closed on an independently authored exception clause", () => {
    const blindDirectory = path.resolve(__dirname, "../fixtures/blind");
    ...
    expect(first.summary.expectationMismatchCount).toBe(1);
    expect(first.summary.expectationMismatches).toEqual([
      "blind-operations-contract::original",
    ]);
